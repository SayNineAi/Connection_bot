...
#
def remove_apostrof(text: str):
    new_text = []
    hj = text.split()
    for i in hj:
        ap_i = i.find("'")
        if ap_i != -1:
            new_text.append(i[ap_i+1:])
        else:
            kl_i = i.find("`")
            if kl_i != -1:
                new_text.append(i[kl_i+1:])
            else:
                kl_i = i.find("’")
                if kl_i != -1:
                    new_text.append(i[kl_i+1:])
...
#
def remove_after_storaket(text: str):
    if "," in text:
        spliltted_text = text.split()
        if len(spliltted_text) > 2:
            ind = text.find(',')
            if ind != -1:
                return text[:ind]
    return text
...
def give_emoji_free_text(text):
    try:
        clean_text = clean.clean(text, lower=False, no_emoji=True)
    except Exception:
        clean_text = clean(text, lower=False, no_emoji=True)
    return clean_text


def remove_dr_chars(text):
    splitted_text = text.split()
    cleared = []
    for i in splitted_text:
        if i.lower() not in ["dr.", "de", "da", "dr"]:
            cleared.append(i)
    result = ""
...
def process_string(text):
    ...
    text_without_apostrof = remove_apostrof(text_without_emojis)
    # print(text_without_apostrof)
    text_without_drda = remove_dr_chars(text_without_apostrof)
    # print(text_without_drda)
    text_without_dot_name = remove_dot_char(text_without_drda)
    # print(text_without_dot_name)
    text_without_after_storaket = remove_after_storaket(text_without_dot_name)
    cleaned_text = remove_punctuation(text_without_after_storaket)
...
#
def csv_to_dict(file: str) -> pd.DataFrame:
    print(file)
...
def artdeco_card__with_hover(driver, wait, employee_count, unavailable):
    ...
    while True:
        ...
        try:
            ...
            print("link_positions 342", len(link_positions))
            break
...
def dict_to_csv(dict, username, number):
    ...
    if not os.path.isfile(r"./data/output/{}_outdata_{}.csv".format(username, number)):
        ...
        csv_file_path = r"./data/output/{}_outdata_{}.csv".format(username, number)
    ...
    try:
        more_button = wait.until(
            EC.presence_of_element_located(
                (By.CLASS_NAME, "org-overflow-menu__dropdown-trigger")
            )
        )
        more_button.click()
        more_parents = wait.until(
            EC.presence_of_all_elements_located(
                (By.CLASS_NAME, "artdeco-dropdown--placement-bottom")
            )
        )
    except TimeoutException:
        return link
...
def final_part(driver, wait, pattern, username, init_data):
    ...
    for index, row in df.iterrows():
        if pd.isna(row["domain"]):
            ...
            if link:
                ...
                df.to_csv(
                    r"data/output/{}_outdata.csv".format(username),
                    index=False,
                    encoding="utf-8",
                )
            else:
    ...
    df.to_csv(
        r"data/output/{}_outdata.csv".format(username), index=False, encoding="utf-8"
    )
    return True
